...
`geometry/` saf ve NGL'den bağımsız tam hassasiyetli hesaplar; `structure/` protein atom çözümleme; `lessons/` iki dilli veri; `state/` seçim ve ilerleme; `components/` erişilebilir React arayüzüdür. Faz 1 küçük olduğu için seçim durumu bileşende yereldir; kapsam büyüyünce reducer'a taşınacaktır.
